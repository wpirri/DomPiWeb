...
int main(int /*argc*/, char** /*argv*/, char** env)
{
  ...
  if( pConfig->GetParam("TRACE-ABMEV.CGI", s_trace))
  {
    trace = atoi(s_trace);
  }
  ...
  {
    strcpy(funcion_call, "dompi_ev_get");
  }
  else if( !strcmp(funcion, "add"))
  {
    strcpy(funcion_call, "dompi_ev_add");
  }
  else if( !strcmp(funcion, "update"))
  {
    strcpy(funcion_call, "dompi_ev_update");
  }
  else if( !strcmp(funcion, "delete"))
  {
    strcpy(funcion_call, "dompi_ev_delete");
  }
  ...
  {
    strcpy(funcion_call, "dompi_ev_list");
  }
  ...
  rc = pClient->Call(funcion_call, query, response, 1000);
  if(rc == 0)
  {
    fprintf(stdout, "%s\r\n", response.Data());
  }
