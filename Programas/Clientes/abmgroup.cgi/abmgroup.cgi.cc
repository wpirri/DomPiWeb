...
int main(int /*argc*/, char** /*argv*/, char** env)
{
  ...
  if( pConfig->GetParam("TRACE-ABMGROUP.CGI", s))
  {
    trace = atoi(s);
  }
  ...
  if(trace)
  {
    openlog("abmgroup.cgi", 0, LOG_USER);
  ...
  {
    strcpy(funcion_call, "dompi_group_get");
  }
  else if( !strcmp(funcion, "add"))
  {
    strcpy(funcion_call, "dompi_group_add");
  }
  else if( !strcmp(funcion, "update"))
  {
    strcpy(funcion_call, "dompi_group_update");
  }
  else if( !strcmp(funcion, "delete"))
  {
    strcpy(funcion_call, "dompi_group_delete");
  }
  ...
  {
    strcpy(funcion_call, "dompi_group_list");
  }
