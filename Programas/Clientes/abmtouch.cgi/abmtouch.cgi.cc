...
int main(int /*argc*/, char** /*argv*/, char** env)
{
  ...
  if( pConfig->GetParam("TRACE-ABMTOUCH.CGI", s))
  {
    trace = atoi(s);
  }
  ...
  if(trace)
  {
    openlog("abmtouch.cgi", 0, LOG_USER);
  ...
  {
    strcpy(funcion_call, "dompi_touch_get");
  }
  else if( !strcmp(funcion, "add"))
  {
    strcpy(funcion_call, "dompi_touch_add");
  }
  else if( !strcmp(funcion, "update"))
  {
    strcpy(funcion_call, "dompi_touch_update");
  }
  else if( !strcmp(funcion, "delete"))
  {
    strcpy(funcion_call, "dompi_touch_delete");
  }
