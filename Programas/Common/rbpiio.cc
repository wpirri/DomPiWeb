...
RBPiIO::RBPiIO(CGMServerWait *pServer)
{
    if(pServer)
    {
    ...
    */
    ...
    url_set_iostatus = "/cgi-bin/iostatus.cgi";
    url_switch_iostatus = "/cgi-bin/ioswitch.cgi";

    url_set_ioconfig = "/cgi-bin/ioconfig.cgi";
    ...
    */
    ...
    url_get_iostatus = "/cgi-bin/iostatus.htm";

    url_get_config = "/cgi-bin/config.htm";

    url_get_wifi = "/cgi-bin/wifi.cgi";
    url_set_wifi = "/cgi-bin/wifi.cgi";
...
int RBPiIO::GetWifiConfig(const char *raddr, rbpi_config_data *config, void(*fcn)(const char* id, const char* data))
{
    ...
    if(m_pLog) m_pLog->Add(100, "[RBPiIO] Send [%s]", buffer);
    if(q.Query(raddr, 80, buffer, buffer, BUFFER_LEN, m_timeout) > 0)
    {
        if(m_pLog) m_pLog->Add(100, "[RBPiIO] Receive [%s]", buffer);
...
int RBPiIO::SetWifiConfig(const char *raddr, rbpi_config_data *config, void(*fcn)(const char* id, const char* data))
{
    ...
    if(m_pLog) m_pLog->Add(50, "[RBPiIO] Encolando configuracion WiFi para %s [%s]", raddr, buffer);
    return RequestEnqueue(raddr, buffer, fcn);
}

int RBPiIO::GetConfig(const char *raddr, char *msg, int max_msg_len, void(*fcn)(const char* id, const char* data))
{
    int rc;
    cJSON *json_obj;
...
int RBPiIO::SetConfig(const char *raddr, char *msg, void(*fcn)(const char* id, const char* data))
{
    int rc;
    cJSON *json_obj;
    json_obj = cJSON_Parse(msg);
...
int RBPiIO::GetIO(const char *raddr, char *msg, int max_msg_len, void(*fcn)(const char* id, const char* data))
{
    int rc;
    cJSON *json_obj;
    json_obj = cJSON_Parse(msg);
...
int RBPiIO::SetIO(const char *raddr, char* msg, void(*fcn)(const char* id, const char* data))
{
    int rc;
    cJSON *json_obj;
    json_obj = cJSON_Parse(msg);
...
int RBPiIO::SwitchIO(const char *raddr, char* msg, void(*fcn)(const char* id, const char* data))
{
    int rc;
    cJSON *json_obj;
    json_obj = cJSON_Parse(msg);
...
int RBPiIO::PulseIO(const char *raddr, char* msg, void(*fcn)(const char* id, const char* data))
{
    int rc;
    cJSON *json_obj;
    json_obj = cJSON_Parse(msg);
...
int RBPiIO::GetConfig(const char *raddr, cJSON *json_obj, void(*fcn)(const char* id, const char* data))
{
    char buffer[BUFFER_LEN+1];
    char *p;
    CTcp q;
    ...
    if(m_pLog) m_pLog->Add(100, "[RBPiIO] GetConfig Send [%s]", buffer);

    if(q.Query(raddr, 80, buffer, buffer, BUFFER_LEN, m_timeout) > 0)
    {
        if(m_pLog) m_pLog->Add(100, "[RBPiIO] GetConfig Receive [%s]", buffer);
        rc = HttpRespCode(buffer);
        if(rc != 200) return rc;
        /* Me posiciono al final de la cabecera HTTP, al principio de los datos */
        p = strstr(buffer, "\r\n\r\n");
...
int RBPiIO::SetConfig(const char *raddr, cJSON *json_obj, void(*fcn)(const char* id, const char* data))
{
    char buffer[BUFFER_LEN+1];
    char data[1024];
 ...
 //   cJSON *json_flag_DHT2x;
    ...
    if(m_pLog) m_pLog->Add(10, "[RBPiIO] Enviando Config a: %s", raddr);

    cJSON_PrintPreallocated(json_obj, buffer, BUFFER_LEN, 0);
    if(m_pLog) m_pLog->Add(100, "[RBPiIO::SetConfig] raddr: %s json: %s", raddr, buffer);
...
//    }
    ...
    if(m_pLog) m_pLog->Add(100, "[RBPiIO::SetConfig] Ports: %s", buffer);
    ...
    cJSON_ArrayForEach(json_un_obj, json_Ports)
    {
        ...
        if(json_Port && json_Tipo_ASS)
        {
            ...
            {
                if(m_pLog) m_pLog->Add(100, "[RBPiIO::SetConfig] %s = %s", json_Port->valuestring, json_Tipo_ASS->valuestring);
    ...
    if(m_pLog) m_pLog->Add(50, "[RBPiIO] Encolando configuracion I/O para %s [%s]", raddr, buffer);
    return RequestEnqueue(raddr, buffer, fcn);
}

int RBPiIO::SetTime(const char *raddr, void(*fcn)(const char* id, const char* data))
{
    time_t t;
    struct tm *tm_time;
    char buffer[BUFFER_LEN+1];
    ...
    sprintf(buffer, http_post, url_set_ioconfig, raddr, strlen(data), data);
    if(m_pLog) m_pLog->Add(50, "[RBPiIO] Encolando configuracion Fecha y Hora para %s [%s]", raddr, buffer);
    return RequestEnqueue(raddr, buffer, fcn);
}

int RBPiIO::GetIO(const char *raddr, cJSON *json_obj, void(*fcn)(const char* id, const char* data))
{
    char buffer[BUFFER_LEN+1];
    char *p;
    CTcp q;
    ...
    if(m_pLog) m_pLog->Add(100, "[RBPiIO] GetIO Send [%s]", buffer);
    if(q.Query(raddr, 80, buffer, buffer, BUFFER_LEN, m_timeout) > 0)
    {
        if(m_pLog) m_pLog->Add(100, "[RBPiIO] GetIO Receive [%s]", buffer);
        rc = HttpRespCode(buffer);
        if(rc != 200) return rc;
        /* Me posiciono al final de la cabecera HTTP, al principio de los datos */
        p = strstr(buffer, "\r\n\r\n");
...
int RBPiIO::SetIO(const char *raddr, cJSON *json_obj, void(*fcn)(const char* id, const char* data))
{
    char buffer[BUFFER_LEN+1];
    char data[1024];
    char port[16];
    ...
    if(m_pLog) m_pLog->Add(50, "[RBPiIO] Encolando estado de I/O para %s [%s]", raddr, buffer);
    return RequestEnqueue(raddr, buffer, fcn);
}

int RBPiIO::SwitchIO(const char *raddr, cJSON *json_obj, void(*fcn)(const char* id, const char* data))
{
    char buffer[BUFFER_LEN+1];
    char data[1024];
    char port[16];
    ...
    if(m_pLog) m_pLog->Add(50, "[RBPiIO] Encolando switch de I/O para %s [%s]", raddr, buffer);
    return RequestEnqueue(raddr, buffer, fcn);
}

int RBPiIO::PulseIO(const char *raddr, cJSON *json_obj, void(*fcn)(const char* id, const char* data))
{
    char buffer[BUFFER_LEN+1];
    char data[1024];
    char port[16];
    ...
    sprintf(buffer, http_post, url_set_iostatus, raddr, strlen(data), data);
    if(m_pLog) m_pLog->Add(50, "[RBPiIO] Encolando pulso de I/O para %s [%s]", raddr, buffer);
    return RequestEnqueue(raddr, buffer, fcn);
}
...
int RBPiIO::HttpData(const char* http, char* data)
{
    char* p;
...
int RBPiIO::RequestEnqueue(const char* dest, const char* data, void(*fcn)(const char* id, const char* data))
{
    int i;
    queue_data qd;
    ...
    if(i < MAX_QUEUE_COUNT)
    {
        ...
        {
            /* Cola Nueva */
            m_queue_list[i].id = QueueOpen(RBPI_MSG_MAX_QUEUE, sizeof(queue_data), m_queue_list[i].buffer);
...
int RBPiIO::RequestDequeue(const char* dest, queue_data* qdata, unsigned int retry)
{
    CTcp q;
    ...
    if(m_pLog) m_pLog->Add(100, "[RBPiIO] Send [%s] retry: %u", qdata->buffer, retry);
    if(q.Query(dest, m_port, qdata->buffer, buffer, RBPI_MSG_MAX_LEN, 1500) > 0)
    {
        if(m_pLog) m_pLog->Add(100, "[RBPiIO] Receive [%s]", buffer);
        rc = HttpRespCode(buffer);
        if(rc == 200)
