...
int main(/*int argc, char** argv, char** env*/void)
{
	...
	/* Mensajes infrarrojos */
	m_pServer->Suscribe("dompi_msgir_list", GM_MSG_TYPE_CR);
	m_pServer->Suscribe("dompi_msgir_list_all", GM_MSG_TYPE_CR);
	m_pServer->Suscribe("dompi_msgir_get", GM_MSG_TYPE_CR);
	m_pServer->Suscribe("dompi_msgir_add", GM_MSG_TYPE_CR);
	m_pServer->Suscribe("dompi_msgir_delete", GM_MSG_TYPE_CR);
	m_pServer->Suscribe("dompi_msgir_update", GM_MSG_TYPE_CR);
	...
	while((rc = m_pServer->Wait(fn, typ, message, 4096, &message_len, (-1) )) >= 0)
	{
		if(rc > 0)
		{
			...
			*		dompi_msgir_list
			**************************************************************** */
			else if( !strcmp(fn, "dompi_msgir_list"))
			{
				json_Message = cJSON_Parse(message);
				message[0] = 0;

				json_Query_Result = cJSON_CreateArray();
				strcpy(query, "SELECT Id, Nombre, Tipo_Dispositivo "
								"FROM TB_DOM_MSG_IR "
								"ORDER BY Nombre ASC;");
				m_pServer->m_pLog->Add(100, "[QUERY][%s]", query);
				rc = pDB->Query(json_Query_Result, query);
			...
			/* ****************************************************************
			*		dompi_msgir_list_all
			**************************************************************** */
			else if( !strcmp(fn, "dompi_msgir_list_all"))
			{
				json_Message = cJSON_Parse(message);
				message[0] = 0;
			...
			*		dompi_msgir_get
			**************************************************************** */
			else if( !strcmp(fn, "dompi_msgir_get"))				
			{
				json_Message = cJSON_Parse(message);
				message[0] = 0;
				json_Id = cJSON_GetObjectItemCaseSensitive(json_Message, "Id");
				...
				{
					json_Query_Result = cJSON_CreateArray();
					sprintf(query, "SELECT * FROM TB_DOM_MSG_IR WHERE Id = %s;", json_Id->valuestring);
					m_pServer->m_pLog->Add(100, "[QUERY][%s]", query);
					rc = pDB->Query(json_Query_Result, query);
					m_pServer->m_pLog->Add((pDB->LastQueryTime()>1)?1:100, "[QUERY] rc= %i, time= %li [%s]", rc, pDB->LastQueryTime(), query);
					if(rc < 0) m_pServer->m_pLog->Add(1, "[QUERY] ERROR [%s] en [%s]", pDB->m_last_error_text, query);
			...
			*		dompi_msgir_add
			**************************************************************** */
			else if( !strcmp(fn, "dompi_msgir_add"))
			{
				json_Message = cJSON_Parse(message);
				...
				/* Obtengo un ID para el elemento nuevo y lo cambio en el dato recibido */
				temp_l = pDB->NextId("TB_DOM_MSG_IR", "Id");
				sprintf(temp_s, "%li", temp_l);
				cJSON_DeleteItemFromObjectCaseSensitive(json_Message, "Id");
				cJSON_AddStringToObject(json_Message, "Id", temp_s);
				...
				sprintf(query, "INSERT INTO TB_DOM_MSG_IR %s VALUES %s;", query_into, query_values);
				m_pServer->m_pLog->Add(100, "[QUERY][%s]", query);
				rc = pDB->Query(NULL, query);
				m_pServer->m_pLog->Add((pDB->LastQueryTime()>1)?1:100, "[QUERY] rc= %i, time= %li [%s]", rc, pDB->LastQueryTime(), query);
				if(rc < 0) m_pServer->m_pLog->Add(1, "[QUERY] ERROR [%s] en [%s]", pDB->m_last_error_text, query);
			...
			*		dompi_msgir_delete
			**************************************************************** */
			else if( !strcmp(fn, "dompi_msgir_delete"))
			{
				json_Message = cJSON_Parse(message);
				...
				if(json_Id)
				{
					if( atoi(json_Id->valuestring) != 0 )
					{
						sprintf(query, "DELETE FROM TB_DOM_MSG_IR WHERE Id = %s;", json_Id->valuestring);
						m_pServer->m_pLog->Add(100, "[QUERY][%s]", query);
						rc = pDB->Query(NULL, query);
						m_pServer->m_pLog->Add((pDB->LastQueryTime()>1)?1:100, "[QUERY] rc= %i, time= %li [%s]", rc, pDB->LastQueryTime(), query);
						if(rc < 0) m_pServer->m_pLog->Add(1, "[QUERY] ERROR [%s] en [%s]", pDB->m_last_error_text, query);
			...
			*		dompi_msgir_update
			**************************************************************** */
			else if( !strcmp(fn, "dompi_msgir_update"))
			{
				json_Message = cJSON_Parse(message);
				...
				if(strlen(query_where))
				{
					sprintf(query, "UPDATE TB_DOM_MSG_IR SET %s WHERE %s;", query_values, query_where);
					m_pServer->m_pLog->Add(100, "[QUERY][%s]", query);
					rc = pDB->Query(NULL, query);
					m_pServer->m_pLog->Add((pDB->LastQueryTime()>1)?1:100, "[QUERY] rc= %i, time= %li [%s]", rc, pDB->LastQueryTime(), query);
					if(rc < 0) m_pServer->m_pLog->Add(1, "[QUERY] ERROR [%s] en [%s]", pDB->m_last_error_text, query);
...
void OnClose(int sig)
{
	...
	/* Mensajes infrarrojos */
	m_pServer->UnSuscribe("dompi_msgir_list", GM_MSG_TYPE_CR);
	m_pServer->UnSuscribe("dompi_msgir_list_all", GM_MSG_TYPE_CR);
	m_pServer->UnSuscribe("dompi_msgir_get", GM_MSG_TYPE_CR);
	m_pServer->UnSuscribe("dompi_msgir_add", GM_MSG_TYPE_CR);
	m_pServer->UnSuscribe("dompi_msgir_delete", GM_MSG_TYPE_CR);
	m_pServer->UnSuscribe("dompi_msgir_update", GM_MSG_TYPE_CR);
